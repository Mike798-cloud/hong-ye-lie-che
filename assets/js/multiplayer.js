...
function nowLabel(){return new Date().toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit',hour12:false})}
...
Storage.prototype.setItem=function(key,value){
  ...
  if(!suppressSaveHook&&this===localStorage&&key===SAVE_KEY){queueMicrotask(onLocalGameSave)}
  return result;
};
...
async function api(path,options={}){
  ...
  const res=await fetch(base+path,{method:options.method||'GET',headers:{'Content-Type':'application/json'},body:options.body?JSON.stringify(options.body):undefined,cache:'no-store'});
...
async function createRoom(name){
  ...
  finally{busy=false;renderPanel();injectUI()}
}
...
function connect(){
  clearTimeout(reconnectTimer);if(!session)return;
  const base=serverForSession();if(!base){runtime.status='offline';runtime.error='请先在 assets/js/multiplayer-config.js 中填写 Cloudflare Worker 地址。';renderPanel();injectUI();return}
  try{socket?.close()}catch(_){}
  runtime.status='connecting';runtime.error='';injectUI();renderPanel();firstSnapshotPending=true;
  ...
  try{socket=new WebSocket(url)}catch(e){runtime.status='offline';runtime.error=e.message||'无法建立联机连接';scheduleReconnect();return}
  socket.onopen=()=>{
    runtime.status='online';injectUI();renderPanel();
    const full=readSave();
  ...
  socket.onmessage=e=>{let m;try{m=JSON.parse(e.data)}catch(_){return}handleMessage(m)};
  socket.onclose=e=>{runtime.status='offline';runtime.players=[];if(e.code===4404)runtime.error='房间已失效或不存在。';else if(e.code===4403)runtime.error='联机身份验证失败，请退出房间后重新加入。';else if(e.code===4409)runtime.error='该账号已在另一个页面连接。';injectUI();renderPanel();scheduleReconnect()};
  socket.onerror=()=>{};
}
function scheduleReconnect(){if(!session||reconnectTimer)return;reconnectTimer=setTimeout(()=>{reconnectTimer=0;connect()},RECONNECT_DELAY)}
function handleMessage(m){
  ...
  if(m.type==='snapshot'){
    ...
    if(signature(local)!==signature(remote)){
      const next={...full,...remote,started:true};writeSave(next);injectUI();renderPanel();setTimeout(()=>location.reload(),90);return;
    }
    if(wasFresh)toast(`已进入调查组 ${session.roomCode}`,'good');injectUI();renderPanel();schedulePresence(full);return;
  }
  if(m.type==='presence'){runtime.players=Array.isArray(m.players)?m.players:[];const me=runtime.players.find(p=>p.playerId===session?.playerId);if(me&&session&&session.host!==!!me.host){session.host=!!me.host;persistSession()}injectUI();renderPanel();return}
  if(m.type==='feed'){
...
function leaveRoom(){
  ...
  try{socket?.close(1000,'leave')}catch(_){}socket=null;clearSession();restoreSingle();runtime={status:'idle',players:[],feed:[],revision:0,error:'',roomExists:true};panelOpen=false;location.reload();
}
...
function injectUI(){
  ...
  if(cover){let b=cover.querySelector('[data-mp-open]');if(!b){b=document.createElement('button');b.type='button';b.className='btn mp-cover-btn';b.dataset.mpOpen='1';const help=cover.querySelector('[data-help]');cover.insertBefore(b,help||null)}b.textContent=session?`调查组 ${session.roomCode}`:'联机调查'}
  const top=document.querySelector('.top-actions');
  if(top){let b=top.querySelector('[data-mp-panel]');if(!b){b=document.createElement('button');b.type='button';b.dataset.mpPanel='1';top.insertBefore(b,top.querySelector('[data-audio]')||null)}const cls=session?runtime.status:'idle';b.className=`mp-team-button ${cls==='online'?'online':cls==='connecting'?'connecting':'offline'}`;const count=runtime.players.filter(p=>p.online).length;b.innerHTML=`<span class="mp-dot"></span><span class="mp-team-label">${session?`调查组 ${count||1}/${MAX_PLAYERS}`:'联机调查'}</span>`}
}
...
document.addEventListener('click',e=>{
  ...
  if(t){
    ...
    if(t.matches('[data-mp-reconnect]'))return connect();
...
document.addEventListener('DOMContentLoaded',()=>{
  observer.observe(document.getElementById('app')||document.body,{childList:true,subtree:true});injectUI();
